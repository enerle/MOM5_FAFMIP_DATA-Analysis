...
temptend_sum_glb    = [None]*len(filename_glb)
...
temptend_sum_soc    = [None]*len(filename_soc)
...
temptend_sum_atl    = [None]*len(filename_atl)
...
temptend_sum_ipa    = [None]*len(filename_ipa)
...
temptend_sum_pac    = [None]*len(filename_pac)
...
temptend_sum_ind    = [None]*len(filename_ind)
...
for i in range(len(filename_glb)):
    ...
    file.close()
for i in range(len(filename_glb)):
    temptend_total_glb[i]  = np.squeeze(np.mean(dTdt_total_glb[i][61:70]))
    temptend_added_glb[i]  = np.squeeze(np.mean(dTdt_added_glb[i][61:70])  - dTdt_added_glb[i][0])
    temptend_redist_glb[i] = np.squeeze(np.mean(dTdt_redist_glb[i][61:70]) - np.mean(dTdt_total_glb[-1][61:70]))
    temptend_sum_glb[i]    = temptend_redist_glb[i] + temptend_added_glb[i]
...
for i in range(len(filename_soc)):
    ...
    file.close()
for i in range(len(filename_soc)):
    temptend_total_soc[i]  = np.squeeze(np.mean(dTdt_total_soc[i][61:70]))
    temptend_added_soc[i]  = np.squeeze(np.mean(dTdt_added_soc[i][61:70])  - dTdt_added_soc[i][0])
    temptend_redist_soc[i] = np.squeeze(np.mean(dTdt_redist_soc[i][61:70]) - np.mean(dTdt_total_soc[-1][61:70]))
    temptend_sum_soc[i]    = temptend_redist_soc[i] + temptend_added_soc[i]
...
for i in range(len(filename_atl)):
    ...
    file.close()
for i in range(len(filename_atl)):
    temptend_total_atl[i]  = np.squeeze(np.mean(dTdt_total_atl[i][61:70]))
    temptend_added_atl[i]  = np.squeeze(np.mean(dTdt_added_atl[i][61:70])  - dTdt_added_atl[i][0])
    temptend_redist_atl[i] = np.squeeze(np.mean(dTdt_redist_atl[i][61:70]) - np.mean(dTdt_total_atl[-1][61:70]))
    temptend_sum_atl[i]    = temptend_redist_atl[i] + temptend_added_atl[i]
...
for i in range(len(filename_ipa)):
    ...
    file.close()
for i in range(len(filename_ipa)):
    temptend_total_ipa[i]  = np.squeeze(np.mean(dTdt_total_ipa[i][61:70]))
    temptend_added_ipa[i]  = np.squeeze(np.mean(dTdt_added_ipa[i][61:70])  - dTdt_added_ipa[i][0])
    temptend_redist_ipa[i] = np.squeeze(np.mean(dTdt_redist_ipa[i][61:70]) - np.mean(dTdt_total_ipa[-1][61:70]))
    temptend_sum_ipa[i]    = temptend_redist_ipa[i] + temptend_added_ipa[i]
...
for i in range(len(filename_pac)):
    ...
    file.close()
for i in range(len(filename_pac)):
    temptend_total_pac[i]  = np.squeeze(np.mean(dTdt_total_pac[i][61:70]))
    temptend_added_pac[i]  = np.squeeze(np.mean(dTdt_added_pac[i][61:70])  - dTdt_added_pac[i][0])
    temptend_redist_pac[i] = np.squeeze(np.mean(dTdt_redist_pac[i][61:70]) - np.mean(dTdt_total_pac[-1][61:70]))
    temptend_sum_pac[i]    = temptend_redist_pac[i] + temptend_added_pac[i]
...
for i in range(len(filename_ind)):
    ...
    file.close()
for i in range(len(filename_ind)):
    temptend_total_ind[i]  = np.squeeze(np.mean(dTdt_total_ind[i][61:70]))
    temptend_added_ind[i]  = np.squeeze(np.mean(dTdt_added_ind[i][61:70])  - dTdt_added_ind[i][0])
    temptend_redist_ind[i] = np.squeeze(np.mean(dTdt_redist_ind[i][61:70]) - np.mean(dTdt_total_ind[-1][61:70]))
    temptend_sum_ind[i]    = temptend_redist_ind[i] + temptend_added_ind[i]

#rc('text', usetex=True)
rc('figure', figsize=(8.27,11.69))

colors = ["black","red","blue","white"]
labels = ["total","added","redist","redist+added"]
...
for i in range(len(exp)-1):
    ...
    ax.plot(0.8,temptend_added_glb[i],                       marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[1]) #added
    ax.plot(1.2,temptend_redist_glb[i],                      marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[2]) #redist
    ax.plot(1.0,temptend_sum_glb[i],                         marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[3]) #total=redist+added

    ax.plot(3.0,temptend_total_soc[i]-temptend_total_soc[-1],marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[0])
    ax.plot(2.8,temptend_added_soc[i],                       marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[1])
    ax.plot(3.2,temptend_redist_soc[i],                      marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[2])
    ax.plot(3.0,temptend_sum_soc[i],                         marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[3]) #total=redist+added

    ax.plot(5.0,temptend_total_atl[i]-temptend_total_atl[-1],marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[0])
    ax.plot(4.8,temptend_added_atl[i],                       marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[1])
    ax.plot(5.2,temptend_redist_atl[i],                      marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[2])
    ax.plot(5.0,temptend_sum_atl[i],                         marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[3]) #total=redist+added

    ax.plot(7.0,temptend_total_ipa[i]-temptend_total_ipa[-1],marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[0])
    ax.plot(6.8,temptend_added_ipa[i],                       marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[1])
    ax.plot(7.2,temptend_redist_ipa[i],                      marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[2])
    ax.plot(7.0,temptend_sum_ipa[i],                         marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[3]) #total=redist+added

    ax.plot(9.0,temptend_total_pac[i]-temptend_total_pac[-1],marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[0])
    ax.plot(8.8,temptend_added_pac[i],                       marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[1])
    ax.plot(9.2,temptend_redist_pac[i],                      marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[2])
    ax.plot(9.0,temptend_sum_pac[i],                         marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[3]) #total=redist+added
    ...
    ax.plot(10.8,temptend_added_ind[i],                       marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[1],label=labels[1])
    ax.plot(10.8,temptend_redist_ind[i],                      marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[2],label=labels[2])
    ax.plot(11.0,temptend_sum_ind[i],                         marker='o',markersize=8,color='k',linewidth=0,markerfacecolor=colors[3]) #total=redist+added

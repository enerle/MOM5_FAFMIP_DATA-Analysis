...
#---------------------- LOAD ALL DATA
datadir = '/home/netapp-clima-users1/rnavarro/ANALYSIS/BLAKER/TEMP-tendency/DATA_vInt_MLD_BSO_PAC'
datadir2 = '/home/clima-archive2/rfarneti/RENE/DATA'

exp      = ["Stress","Water","Heat","flux-only"]
filename = ["STC_temptend_FAFSTRESS_PAC_zonal.nc","STC_temptend_FAFWATER_PAC_zonal.nc","STC_temptend_FAFHEAT_PAC_zonal.nc","STC_temptend_flux-only_PAC_zonal.nc"]
filename2  = ["BSO_PAC_FAFSTRESS.nc","BSO_PAC_FAFWATER.nc","BSO_PAC_FAFHEAT.nc","BSO_PAC_flux-only.nc"]
...
for i in range(len(filename)):
    ...
    dia_swp[i]   = diapycnal_mix[i] + sw_heat[i]
...
for i in range(len(exp)-1): 
    ...
    plt.axvline(x=30,color='k',linestyle='--',linewidth=1.5); plt.axvline(x=-33.5,color='k',linestyle='--',linewidth=1.5)
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],residual[i][:,:]-residual[-1][:,:],shading='flat',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],resolved[i][:,:]-resolved[-1][:,:],shading='flat',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],eddy[i][:,:]-eddy[-1][:,:],shading='flat',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],dia_swp[i][:,:]-dia_swp[-1][:,:],shading='flat',cmap=cmap2)
    plt.clim(kmin,kmax)
    ...
    cs  = plt.pcolormesh(lat2[i],depth2[i],isopycnal_mix[i][:,:]-isopycnal_mix[-1][:,:],shading='flat',cmap=cmap2)
    plt.clim(kmin,kmax)

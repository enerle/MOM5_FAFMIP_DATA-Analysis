...
let taux = tau_x[d=1]*umask_pac
let temp = sst[d=2]*tmask_pac
...
let dlon_diff_pac = dlon_diff*tmask_pac
let dx            = dlon_diff_pac*rad*Ry

let dlat          = geolat_t[i=1:360,j=1:200,d=4]
let dlat_diff     = dlat[y=@SHF:1] - dlat
let dlat_diff_pac = dlat_diff*tmask_pac
let dy            = dlat_diff_pac*rad*R
...
let IDD                         = if(y[gy=trans_dx_tempgrad_dy] GT tauxminlat) then 0 else 1
let trans_dx_tempgrad_dy_IDD    = trans_dx_tempgrad_dy[j=1:200]*IDD[j=1:200]
let cumsum_trans_dx_tempgrad_dy = trans_dx_tempgrad_dy_IDD[y=10:35@rsum] - trans_dx_tempgrad_dy_IDD[y=10:35@sum] 
let estc                        = ABS(cp*cumsum_trans_dx_tempgrad_dy)*1.e-15 !!PW

let ekman_trans = (trans_dx/rho)*1.e-6 !m3/s

save/file=STC_trans_energy_${EXP}.nc/clobber trans_dx,ekman_trans
save/file=STC_trans_energy_${EXP}.nc/append  dx[i=@sum],dy[i=@ave]
save/file=STC_trans_energy_${EXP}.nc/append  tempgrad[i=@ave]
save/file=STC_trans_energy_${EXP}.nc/append  taux[i=@ave],temp[i=@ave]
save/file=STC_trans_energy_${EXP}.nc/append  estc

...
filename4  = ["BSO_PAC_FAFSTRESS.nc","BSO_PAC_FAFWATER.nc","BSO_PAC_FAFHEAT.nc","BSO_PAC_FAFALL.nc","BSO_PAC_flux-only.nc"]
...
#    lat_bso[i]   = file.variables['Y'][:]
    bso_pac[i]       = file.variables['field'][:]
    file.close()
...
for i in range(len(exp)-1):
    ...
    cs  = plt.contourf(LAT2[-1],DEPTH2[-1],PMOC[i]-PMOC[-1],levels=clevs,cmap=cmap2,extend='both')

    #plt.clim(kmin,kmax)

    cc2 = plt.contour(LAT2[i],DEPTH2[i],PMOC[-1],levels=clevs_moc,colors='black',linestyles='solid',linewidths=1)
    cc3 = plt.contour(LAT2[i],DEPTH2[i],POTRHO_pac[i],levels=clevs_rho,colors='gray',linestyles='dashed',linewidths=1)
    ...
    #plt.plot(LAT[i],bso_pac[i][:],linestyle='solid',color='red',linewidth=1.5)
...
plt.savefig('MOC_PAC_change_tropical_area.png',transparent = True, bbox_inches='tight',dpi=600)
#plt.show()

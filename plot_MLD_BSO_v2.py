...
exp       = ["Stress","Water","Heat","All","Stress_Heat","flux-only"]
filename  = ["MLD_BSO_FAFSTRESS.nc","MLD_BSO_FAFWATER.nc","MLD_BSO_FAFHEAT.nc","MLD_BSO_FAFALL.nc","MLD_BSO_FAFHEAT_FAFSTRESS.nc","MLD_BSO_flux-only.nc"]
filename2  = ["MOC_FAFSTRESS.nc","MOC_FAFWATER.nc","MOC_FAFHEAT.nc","MOC_FAFALL.nc","MOC_FAFHEAT_FAFSTRESS.nc","MOC_flux-only.nc"] 
filename3 = ["pot_rho_0_zonalmean_flux-only.nc","pot_rho_0_zonalmean_FAFSTRESS.nc","pot_rho_0_zonalmean_FAFWATER.nc","pot_rho_0_zonalmean_FAFHEAT.nc","pot_rho_0_zonalmean_FAFALL.nc","pot_rho_0_zonalmean_FAFHEAT_FAFSTRESS.nc"]
...
for i in range(len(exp)):
    for j in range(len(LAT[i])):
        pmocj = PMOC[i][:,j]
        for k in range(4,32): #len(pmocj)):
            if LAT[i][j]>=0.0:
#                if pmocj[k]<=2. and pmocj[k]>=0.:
                if pmocj[k]<=1. and pmocj[k]>=-1.:
#                if pmocj[k]<=2. and pmocj[k]>=1.:
#                if pmocj[k]<=10. and pmocj[k]>=8.:
